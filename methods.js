...
function SetParameterValues(device, request, callback) {
  ...
  for (let p of parameterValues) {
	...
	console.log("Error setting parameter:" + device[name] + " ,value=" +value.text);
 }
  }//for
...
function Reboot(device, request, callback) {
 ...
 for(let i=1;i<6;i++){
    let pathName="InternetGatewayDevice.BulkData.Profile."+i+".Enable";
    ...
    if(device[pathName][1]==='true'){
        ...
        else
        {
			...
			if(kpiName.includes("ErrorsSent") || kpiName.includes("ErrorsReceived")){
			   pathName="InternetGatewayDevice.BulkData.Profile."+i+".Parameter."+iparam+".ValueFunction";
			   device[pathName][1]="v.stableVal(0)";
			}

			if(kpiName.includes("CPUUsage")){
                           pathName="InternetGatewayDevice.BulkData.Profile."+i+".Parameter."+iparam+".ValueFunction";
                           device[pathName][1]="v.randomVal(1,30)";
                        }

			if(kpiName.includes("MemoryStatus")){
                           pathName="InternetGatewayDevice.BulkData.Profile."+i+".Parameter."+iparam+".ValueFunction";
                           device[pathName][1]="v.randomVal(60000,70000)";
                        }
  ...
  let response = xmlUtils.node("cwmp:RebootResponse", {}, xmlUtils.node("Status", {}, "0"));	
  return callback(response);
}
